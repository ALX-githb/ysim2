	...
	private void Update()
	{
		if (!PauseScreen.Show)
		{
			if (Yandere.CanMove && !Yandere.Aiming)
			{
				...
				if (!Yandere.EasterEggMenu.activeInHierarchy && (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Alpha1) || ControlFreak2.CF2Input.GetKeyDown(KeyCode.Alpha2) || ControlFreak2.CF2Input.GetKeyDown(KeyCode.Alpha3) || ControlFreak2.CF2Input.GetKeyDown(KeyCode.Alpha4) || ControlFreak2.CF2Input.GetKeyDown(KeyCode.Alpha5)))
				{
					Yandere.EmptyHands();
		...
		else
		{
			...
			if (!Yandere.CanMove || Yandere.Aiming || PauseScreen.Show || InputDevice.Type == InputDeviceType.MouseAndKeyboard)
			{
				Show = false;
			}
		}
		...
		if (!Yandere.CanMove || Yandere.Aiming || PauseScreen.Show || InputDevice.Type == InputDeviceType.Gamepad || ControlFreak2.CF2Input.GetButton("Y"))
		{
			KeyboardShow = false;
		}
	}
