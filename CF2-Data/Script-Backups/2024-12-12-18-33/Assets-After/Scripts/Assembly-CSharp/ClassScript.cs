...
public class ClassScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (Show)
		{
			...
			if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Backslash))
			{
				GivePoints();
			}
			...
			if (ControlFreak2.CF2Input.GetButtonDown("A") && StudyPoints == 0)
			{
				Show = false;
		...
		else if (GradeUpWindow.localScale.x > 0.99f && ControlFreak2.CF2Input.GetButtonDown("A"))
		{
			PromptBar.ClearButtons();
			GradeUp = false;
		}
