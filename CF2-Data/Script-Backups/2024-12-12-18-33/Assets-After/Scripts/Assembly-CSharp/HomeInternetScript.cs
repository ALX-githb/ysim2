...
public class HomeInternetScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (!HomeYandere.CanMove && !PauseScreen.Show)
		{
			...
			if (!PostSequence)
			{
				if (ControlFreak2.CF2Input.GetButtonDown("A") && WriteIcon.activeInHierarchy && !Posted)
				{
					if (!ShowMenu)
					{
						if (!WritingPost)
				...
				if (ControlFreak2.CF2Input.GetButtonDown("X") && PostIcon.activeInHierarchy)
				{
					...
					PostSequence = true;
					Posted = true;
					if (Student == 7 && Location == 7 && Action == 9)
					{
						Success = true;
					}
				}
