...
public class DatingMinigameScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		else if (LoveManager.RivalWaiting)
		{
			if (Rival == null)
			{
				Suitor = StudentManager.Students[13];
				Rival = StudentManager.Students[7];
			}
		...
		else if (Phase == 4)
		{
			...
			if (!SelectingTopic && !Complimenting && !ShowingOff && !GivingGift)
			{
				...
				if (ControlFreak2.CF2Input.GetButtonDown("A") && Labels[Selected].color.a == 1f)
				{
					if (Selected == 1)
					{
						SelectingTopic = true;
			...
			else if (SelectingTopic)
			{
				...
				if (ControlFreak2.CF2Input.GetButtonDown("A") && TopicIcons[TopicSelected].color.a == 1f)
				{
					...
					DetermineOpinion();
					if (!ConversationGlobals.GetTopicLearnedByStudent(Opinion, 7))
					{
						ConversationGlobals.SetTopicLearnedByStudent(Opinion, 7, true);
					}
					if (Negative)
					{
			...
			else if (Complimenting)
			{
				...
				if (ControlFreak2.CF2Input.GetButtonDown("A") && ComplimentLabels[ComplimentSelected].color.a == 1f)
				{
		...
		else if (Phase == 5)
		{
			...
			if (Speed > 100f && ControlFreak2.CF2Input.GetButtonDown("A"))
			{
				Phase++;
			}
		}
	...
	{
		int[] topics = JSON.Topics[7].Topics;
