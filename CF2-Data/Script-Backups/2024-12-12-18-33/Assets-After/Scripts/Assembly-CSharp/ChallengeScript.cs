	...
	private void Update()
	{
		if (!Viewing)
		{
			if (!Switch)
			{
				...
				if (InputManager.DPadRight || ControlFreak2.CF2Input.GetKey(KeyCode.RightArrow))
				{
				...
				if (InputManager.DPadLeft || ControlFreak2.CF2Input.GetKey(KeyCode.LeftArrow))
				{
					transform.localPosition = new Vector3(transform.localPosition.x + Time.deltaTime * 1000f, transform.localPosition.y, transform.localPosition.z);
				}
				transform.localPosition = new Vector3(transform.localPosition.x + ControlFreak2.CF2Input.GetAxis("Horizontal") * -10f, transform.localPosition.y, transform.localPosition.z);
		...
		if (!Switch && ControlFreak2.CF2Input.GetButtonDown("A") && List == 1)
		{
			Viewing = true;
		}
