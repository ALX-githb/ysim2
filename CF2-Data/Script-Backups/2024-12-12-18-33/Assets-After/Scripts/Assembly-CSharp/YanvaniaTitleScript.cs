...
public class YanvaniaTitleScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (!Logo.gameObject.activeInHierarchy && ControlFreak2.CF2Input.GetKeyDown(KeyCode.M))
		{
		...
		if (!FadeOut)
		{
			...
			else
			{
				...
				if (!YanvaniaGlobals.MidoriEasterEgg)
				{
					if (YanvaniaGlobals.DraculaDefeated)
					{
						...
						if (Prologue.localPosition.y > 2500f)
						{
							...
							if (!ErrorLeave)
							{
								...
								if (ErrorWindow.transform.localScale.x > 0.9f && ControlFreak2.CF2Input.anyKeyDown)
								{
					...
					if ((Controls.alpha == 1f || Credits.alpha == 1f) && ControlFreak2.CF2Input.GetButtonDown("B"))
					{
