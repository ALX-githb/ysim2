...
public class ShutterScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (Snapping)
		{
			if (Close)
			{
				currentPercent += 40f * Time.unscaledDeltaTime;
		...
		else if (Yandere.Aiming)
		{
			...
			if (Timer > 0.5f)
			{
				if (Physics.Raycast(SmartphoneCamera.transform.position, SmartphoneCamera.transform.TransformDirection(Vector3.forward), out hit, float.PositiveInfinity, OnlyPhotography))
				{
								...
								if (!FaceStudent.CameraReacting)
								{
									if (FaceStudent.enabled && !FaceStudent.Stop)
									{
										...
										else if (FaceStudent.PhotoPatience > 0f)
										{
											...
											{
												if (Yandere.Bloodiness > 0f || (double)Yandere.Sanity < 33.33333)
												{
					...
					else if (hit.collider.gameObject.name == "Panties" || hit.collider.gameObject.name == "Skirt")
					{
						...
						if (Physics.Raycast(SmartphoneCamera.transform.position, SmartphoneCamera.transform.TransformDirection(Vector3.forward), out hit, float.PositiveInfinity, OnlyCharacters))
						{
		...
		if (!DisplayError)
		{
			if (PhotoIcons.activeInHierarchy && !Snapping && !TextMessages.gameObject.activeInHierarchy)
			{
				...
				if (!Yandere.RivalPhone && ControlFreak2.CF2Input.GetButtonDown("X"))
				{
			...
			else if (PhotoIcons.activeInHierarchy && ControlFreak2.CF2Input.GetButtonDown("B"))
			{
				ResumeGameplay();
			}
		}
	...
	{
		ErrorWindow.transform.localScale = Vector3.zero;
		Yandere.HandCamera.SetActive(false);
	...
	private void CheckPhoto()
	{
		...
		if (Physics.Raycast(SmartphoneCamera.transform.position, SmartphoneCamera.transform.TransformDirection(Vector3.forward), out hit, float.PositiveInfinity, OnlyRagdolls) && hit.collider.gameObject.layer == 11)
		{
	...
	private void ResumeGameplay()
	{
		...
		Yandere.HandCamera.SetActive(true);
