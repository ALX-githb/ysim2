...
public class RPG_Camera : MonoBehaviour
{
	...
	public void GetInput()
	{
		...
		mouseX += ControlFreak2.CF2Input.GetAxis("Mouse X") * mouseSpeed * (Time.deltaTime / Mathf.Clamp(Time.timeScale, 1E-10f, 1E+10f)) * (float)OptionGlobals.Sensitivity * 10f;
		if (flag)
		{
			if (ControlFreak2.CF2Input.GetAxis("Mouse Y") < 0f)
			{
				mouseY -= ControlFreak2.CF2Input.GetAxis("Mouse Y") * mouseSpeed * (Time.deltaTime / Mathf.Clamp(Time.timeScale, 1E-10f, 1E+10f)) * (float)OptionGlobals.Sensitivity * 10f;
			}
		}
		else
		{
		...
		desiredDistance -= ControlFreak2.CF2Input.GetAxis("Mouse ScrollWheel") * mouseScroll;
	...
	{
		float num = ControlFreak2.CF2Input.GetAxis("Horizontal") * RPG_Controller.instance.turnSpeed;
		mouseX += num;
	}
}
