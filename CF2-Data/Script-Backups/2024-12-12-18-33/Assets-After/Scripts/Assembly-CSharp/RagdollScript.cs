...
public class RagdollScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (!Dumped)
		{
			if (StopAnimation && Character.GetComponent<Animation>().isPlaying)
			{
				Character.GetComponent<Animation>().Stop();
			}
			if (!ControlFreak2.CF2Input.GetButtonDown("LB"))
			{
				if (BloodPoolSpawner.gameObject.activeInHierarchy && !Cauterized)
				{
				...
				if (Prompt.Circle[0].fillAmount == 0f)
				{
					...
					if (!Yandere.Chased && Yandere.Chasers == 0)
					{
						...
						{
							Yandere.Character.GetComponent<Animation>().CrossFade("f02_dismember_00");
				...
				if (Prompt.Circle[3].fillAmount == 0f)
				{
					...
					if (!Student.FireEmitters[1].isPlaying)
					{
						...
						Yandere.Character.GetComponent<Animation>().Play("f02_carryLiftA_00");
						Character.GetComponent<Animation>().Play(LiftAnim);
			...
			if (!Prompt.AcceptingInput[1] && ControlFreak2.CF2Input.GetButtonUp("B"))
			{
				Prompt.AcceptingInput[1] = true;
			}
			bool flag = false;
			if (Yandere.Armed && Yandere.EquippedWeapon.WeaponID == 7)
			{
				flag = true;
			}
			...
			Prompt.HideButton[0] = Dragged || Carried || Tranquil || !flag || Nemesis;
		}
		else if (DumpType == RagdollDumpType.Incinerator)
		{
			if (DumpTimer == 0f && Yandere.Carrying)
			{
				Character.GetComponent<Animation>()[DumpedAnim].time = 2.5f;
			}
			Character.GetComponent<Animation>().CrossFade(DumpedAnim);
			DumpTimer += Time.deltaTime;
			if (Character.GetComponent<Animation>()[DumpedAnim].time >= Character.GetComponent<Animation>()[DumpedAnim].length)
			{
				Incinerator.Corpses++;
	...
	private void LateUpdate()
	{
		...
		if (Yandere.DumpTimer < 1f)
		{
			...
			else if (Carried)
			{
				...
				if (axis != 0f || axis2 != 0f)
				{
					Character.GetComponent<Animation>().CrossFade((!ControlFreak2.CF2Input.GetButton("LB")) ? WalkAnim : RunAnim);
				}
				else
				{
					Character.GetComponent<Animation>().CrossFade(IdleAnim);
				}
				Character.GetComponent<Animation>()[IdleAnim].time = Yandere.Character.GetComponent<Animation>()["f02_carryIdleA_00"].time;
				Character.GetComponent<Animation>()[WalkAnim].time = Yandere.Character.GetComponent<Animation>()["f02_carryWalkA_00"].time;
				Character.GetComponent<Animation>()[RunAnim].time = Yandere.Character.GetComponent<Animation>()["f02_carryRunA_00"].time;
			}
		}
		if (Carried)
		{
	...
	public void Dismember()
	{
		...
		for (int i = 0; i < BodyParts.Length; i++)
		{
			...
			gameObject.GetComponent<BodyPartScript>().Sacrifice = Sacrifice;
			if (Yandere.StudentManager.NoGravity)
			{
