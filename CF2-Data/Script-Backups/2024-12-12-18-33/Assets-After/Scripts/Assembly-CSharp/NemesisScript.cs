...
public class NemesisScript : MonoBehaviour
{
	...
	private void Start()
	{
		...
		Student.Reputation = GameObject.Find("Reputation").GetComponent<ReputationScript>();
	...
	private void Update()
	{
		...
		{
			int num = 1;
			while ((Student.StudentManager.Students[num] != null && Student.StudentManager.Students[num].Male) || (num > 32 && num < 56) || (num > 45 && num < 56) || (num > 65 && num < 71) || num == MissionMode.TargetID)
			{
		...
		if (!Dying)
		{
			if (!Attacking)
			{
				...
				{
					if (Yandere.Stance.Current != StanceType.Crouching && Yandere.Stance.Current != StanceType.Crawling && Vector3.Distance(base.transform.position, Yandere.transform.position) < 10f && ControlFreak2.CF2Input.GetButton("LB"))
					{
						MissionMode.LastKnownPosition.position = Yandere.transform.position;
						UpdateLKP();
					}
