...
public class YandereScript : MonoBehaviour
{
	...
	private void UpdateMovement()
	{
		...
		if (CanMove)
		{
			...
			if (!NearSenpai)
			{
				if (!Input.GetButton("A") && !Input.GetButton("B") && !Input.GetButton("X") && !Input.GetButton("Y") && StudentManager.Clock.Timer > 1f && (Input.GetAxis("LT") > 0.5f || Input.GetMouseButton(1)))
				{
					if (Inventory.RivalPhone && Input.GetButtonDown("LB"))
					{
						CharacterAnimation["f02_cameraPose_00"].weight = 0f;
						if (!RivalPhone)
						{
