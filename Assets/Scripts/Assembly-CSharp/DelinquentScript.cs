...
public class DelinquentScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.V) && LongSkirt != null)
		{
			MyRenderer.sharedMesh = LongSkirt;
		}
		if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Space) && Vector3.Distance(Yandere.transform.position, DelinquentManager.transform.position) < 10f)
		{
			Spaces++;
			if (Spaces == 9)
			{
