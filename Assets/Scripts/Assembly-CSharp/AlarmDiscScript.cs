...
public class AlarmDiscScript : MonoBehaviour
{
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		else if (!Student.Nemesis && Student.Alive && !Student.Dying && !Student.Alarmed && !Student.Wet && !Student.Slave && !Student.WitnessedMurder && !Student.WitnessedCorpse && !Student.InEvent && !Student.Following && !Student.Distracting && Student.Actions[Student.Phase] != StudentActionType.Teaching && Student.Actions[Student.Phase] != StudentActionType.SitAndTakeNotes && !Student.GoAway && Student.Routine && !Student.CheckingNote && !Student.SentHome && Student.Persona != PersonaType.Protective && Student.CharacterAnimation != null && SourceRadio.Victim == null)
		{
	...
	private void InvestigateScream()
	{
		...
		if (Student.Pathfinding != null && !Student.Nemesis)
		{
			...
			Student.ReadPhase = 0;
			Student.StopPairing();
			Debug.Log(Student.Name + " was just alarmed by an alarm disc!");
			if (Student.Persona == PersonaType.PhoneAddict || Student.Sleuthing)
			{
				Debug.Log(Student.Name + "'s phone should be active.");
				Student.SmartPhone.SetActive(true);
			}
			else
			{
				Debug.Log(Student.Name + "'s phone should not be active.");
				Student.SmartPhone.SetActive(false);
			}
			Student.OccultBook.SetActive(false);
			Student.Pen.SetActive(false);
