...
public class PoseModeScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (Show)
		{
			...
			{
				if (ControlFreak2.CF2Input.GetButtonDown("A") && OptionLabels[Selected].color.a == 1f)
				{
			...
			{
				if (ControlFreak2.CF2Input.GetAxis("Horizontal") > 0.5f || ControlFreak2.CF2Input.GetAxis("Horizontal") < -0.5f || ControlFreak2.CF2Input.GetAxis("DpadX") > 0.5f || ControlFreak2.CF2Input.GetAxis("DpadX") < -0.5f || ControlFreak2.CF2Input.GetKey(KeyCode.RightArrow) || ControlFreak2.CF2Input.GetKey(KeyCode.LeftArrow))
				{
					CalculateValue();
					if (Selected == 1)
					{
				...
				else if (Selected == 11 && ControlFreak2.CF2Input.GetButtonDown("A"))
				{
					ResetPose();
				}
				if (ControlFreak2.CF2Input.GetButtonDown("B"))
			...
			else if (Customizing)
			{
				...
				else if (ControlFreak2.CF2Input.GetAxis("Horizontal") > 0.5f || ControlFreak2.CF2Input.GetAxis("Horizontal") < -0.5f || ControlFreak2.CF2Input.GetAxis("DpadX") > 0.5f || ControlFreak2.CF2Input.GetAxis("DpadX") < -0.5f || ControlFreak2.CF2Input.GetKey(KeyCode.RightArrow) || ControlFreak2.CF2Input.GetKey(KeyCode.LeftArrow))
				{
					CalculateValue();
		...
		else
		{
			...
			if (Placing && ControlFreak2.CF2Input.GetButtonDown("A"))
			{
	...
	private void UpdateHighlight()
	{
		...
		Highlight.localPosition = new Vector3(Highlight.localPosition.x, 350f - (float)Selected * 50f, Highlight.localPosition.z);
	}
	...
	public void UpdateLabels()
	{
		...
		if (ChoosingAction)
		{
			...
			OptionLabels[6].text = "Release Student";
			Limit = 6;
		}
		else if (ChoosingBodyRegion)
		{
	...
	{
		if (ControlFreak2.CF2Input.GetAxis("Horizontal") > 0.5f || ControlFreak2.CF2Input.GetAxis("Horizontal") < -0.5f)
		{
		...
		else if (ControlFreak2.CF2Input.GetAxis("DpadX") > 0.5f || ControlFreak2.CF2Input.GetAxis("DpadX") < -0.5f)
		{
		...
		{
			Value = (ControlFreak2.CF2Input.GetKey(KeyCode.RightArrow) ? 1 : (-1));
		}
	}
