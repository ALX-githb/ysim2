	...
	private void Update()
	{
		...
		if (S.Interaction == StudentInteractionType.Idle)
		{
			if (!Fake)
			{
				...
				S.Character.GetComponent<Animation>().CrossFade(IdleAnim);
			}
		...
		else if (S.Interaction == StudentInteractionType.Forgiving)
		{
			if (S.TalkTimer == 3f)
			{
				...
				{
					S.Character.GetComponent<Animation>().CrossFade(S.Nod2Anim);
					S.RepRecovery = 5f;
			...
			else
			{
				...
				if (S.Character.GetComponent<Animation>()[S.Nod2Anim].time >= S.Character.GetComponent<Animation>()[S.Nod2Anim].length)
				{
					S.Character.GetComponent<Animation>().CrossFade(IdleAnim);
				}
				if (S.TalkTimer <= 0f)
		...
		else if (S.Interaction == StudentInteractionType.ReceivingCompliment)
		{
			if (S.TalkTimer == 3f)
			{
				if (S.Club != ClubType.Delinquent)
				{
					S.Character.GetComponent<Animation>().CrossFade(S.LookDownAnim);
		...
		else if (S.Interaction == StudentInteractionType.Gossiping)
		{
			if (S.TalkTimer == 3f)
			{
				if (S.Club != ClubType.Delinquent)
				{
					S.Character.GetComponent<Animation>().CrossFade(S.GossipAnim);
					...
					if (!ConversationGlobals.GetTopicLearnedByStudent(15, S.StudentID))
					{
						S.Yandere.NotificationManager.DisplayNotification(NotificationType.Opinion);
						ConversationGlobals.SetTopicLearnedByStudent(15, S.StudentID, true);
					}
				}
				else
				{
			...
			else
			{
				...
				if (S.Character.GetComponent<Animation>()[S.GossipAnim].time >= S.Character.GetComponent<Animation>()[S.GossipAnim].length)
				{
					S.Character.GetComponent<Animation>().CrossFade(IdleAnim);
				}
				if (S.TalkTimer <= 0f)
		...
		else if (S.Interaction == StudentInteractionType.GivingTask)
		{
			if (S.TalkTimer == 100f)
			{
				...
				S.Character.GetComponent<Animation>().CrossFade(S.TaskAnims[S.TaskPhase]);
			...
			if (S.Character.GetComponent<Animation>()[S.CurrentAnim].time >= S.Character.GetComponent<Animation>()[S.CurrentAnim].length)
			{
				S.Character.GetComponent<Animation>().CrossFade(IdleAnim);
			}
			S.TalkTimer -= Time.deltaTime;
		...
		else if (S.Interaction == StudentInteractionType.FollowingPlayer)
		{
			if (S.TalkTimer == 2f)
			{
				if (S.Club != ClubType.Delinquent)
				{
					...
					{
						S.Character.GetComponent<Animation>().CrossFade(S.Nod1Anim);
			...
			else
			{
				...
				if (S.Character.GetComponent<Animation>()[S.Nod1Anim].time >= S.Character.GetComponent<Animation>()[S.Nod1Anim].length)
				{
					S.Character.GetComponent<Animation>().CrossFade(IdleAnim);
				}
				if (S.TalkTimer <= 0f)
			...
			if (S.TalkTimer == 3f)
			{
				if (S.Club != ClubType.Delinquent)
				{
					...
					else
					{
						...
						Grudge = false;
						if (studentScript.Club == ClubType.Delinquent || (S.Bullied && studentScript.Club == ClubType.Bully))
						{
							Grudge = true;
						}
						if (studentScript.Routine && !studentScript.TargetedForDistraction && !studentScript.InEvent && !Grudge)
						{
							S.Character.GetComponent<Animation>().CrossFade(S.Nod1Anim);
		...
		else if (S.Interaction == StudentInteractionType.PersonalGrudge)
		{
			if (S.TalkTimer == 5f)
			{
				if (S.Persona == PersonaType.Coward || S.Persona == PersonaType.Fragile)
				{
					...
					S.Character.GetComponent<Animation>().CrossFade(S.CowardAnim);
					S.TalkTimer = 5f;
				}
				else
				{
					...
					S.Character.GetComponent<Animation>().CrossFade(S.GrudgeAnim);
				}
			}
			else
			{
		...
		else if (S.Interaction == StudentInteractionType.Feeding)
		{
			if (S.TalkTimer == 3f)
			{
				if (S.Club == ClubType.Delinquent)
				{
					S.Character.GetComponent<Animation>().CrossFade(S.IdleAnim);
		...
		if (S.Waiting)
		{
			...
			if (!Fake)
			{
				...
				S.Alarmed = false;
				if (!S.Following && !S.Distracting && !S.Wet)
				{
					S.Routine = true;
				}
				if (!S.Following)
