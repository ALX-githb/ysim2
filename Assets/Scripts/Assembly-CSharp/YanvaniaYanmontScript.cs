...
public class YanvaniaYanmontScript : MonoBehaviour
{
				...
				if (!Cutscene)
				{
					if (grounded)
					{
						if (!Attacking)
						{
							if (!Crouching)
							{
								if (ControlFreak2.CF2Input.GetAxis("VaniaHorizontal") > 0f)
								{
									inputX = 1f;
								}
								else if (ControlFreak2.CF2Input.GetAxis("VaniaHorizontal") < 0f)
								{
									inputX = -1f;
								}
								else
					...
					if (grounded)
					{
						...
						{
							if (ControlFreak2.CF2Input.GetAxis("VaniaVertical") < -0.5f)
							{
							...
							if (Crouching)
							{
								...
								if (!Attacking)
								{
									...
									else if (ControlFreak2.CF2Input.GetAxis("VaniaVertical") > -0.5f && ControlFreak2.CF2Input.GetButton("X"))
									{
										component["f02_yanvaniaCrouchPose_00"].weight = 0f;
										MyController.center = new Vector3(MyController.center.x, 0.75f, MyController.center.z);
										MyController.height = 1.5f;
						...
						if (!toggleRun)
						{
							speed = ((!ControlFreak2.CF2Input.GetKey(KeyCode.LeftShift)) ? walkSpeed : runSpeed);
						}
						...
						if (!ControlFreak2.CF2Input.GetButton("VaniaJump"))
						{
							jumpTimer++;
						}
	...
	private void Update()
	{
		...
		if (!Injured && CanMove && !Cutscene)
		{
			...
			if (ControlFreak2.CF2Input.GetButtonDown("VaniaAttack") && !Attacking)
			{
			...
			if (Attacking)
			{
				...
				else
				{
					...
					if (ControlFreak2.CF2Input.GetAxis("VaniaHorizontal") > -0.5f && ControlFreak2.CF2Input.GetAxis("VaniaHorizontal") < 0.5f && ControlFreak2.CF2Input.GetAxis("VaniaVertical") > -0.5f && ControlFreak2.CF2Input.GetAxis("VaniaVertical") < 0.5f)
					{
					...
					else
					{
						...
						if (ControlFreak2.CF2Input.GetAxis("VaniaHorizontal") > 0.5f)
						{
					...
					if (!ControlFreak2.CF2Input.GetButton("VaniaAttack"))
					{
						StopAttacking();
					}
				}
		...
		if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Equals))
		{
			Time.timeScale += 10f;
		}
		if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Minus))
		{
			Time.timeScale -= 10f;
