	...
	private void Update()
	{
		...
		if (!PauseScreen.Show)
		{
			if (InView)
			{
				...
				DistanceSqr = (vector - position).sqrMagnitude;
				if (DistanceSqr < MaximumDistanceSqr)
				{
					...
					{
						Debug.DrawLine(Yandere.Eyes.position + Vector3.down * Height, RaycastTarget.position, Color.green);
						RaycastHit hitInfo;
						if (Physics.Linecast(Yandere.Eyes.position + Vector3.down * Height, RaycastTarget.position, out hitInfo, BloodMask))
						{
							if (Debugging)
							{
								Debug.Log("We hit a collider named " + hitInfo.collider.name);
							}
						...
						if (Carried || InSight)
						{
							...
							if (DistanceSqr < MinimumDistanceSqr)
							{
								...
								if (Yandere.NearestPrompt == this)
								{
									...
									if (ControlFreak2.CF2Input.GetButton("A"))
									{
										ButtonHeld = 1;
									}
									else if (ControlFreak2.CF2Input.GetButton("B"))
									{
										ButtonHeld = 2;
									}
									else if (ControlFreak2.CF2Input.GetButton("X"))
									{
										ButtonHeld = 3;
									}
									else if (ControlFreak2.CF2Input.GetButton("Y"))
									{
										ButtonHeld = 4;
									}
									else
