	...
	public class ProceduralGridMover : VersionedMonoBehaviour
	{
		...
		private IEnumerator UpdateGraphCoroutine()
		{
			...
			if (Mathf.Abs(offset.x) <= width && Mathf.Abs(offset.y) <= depth)
			{
				...
				int yieldEvery2 = 1000;
				int approxNumNodesToUpdate = Mathf.Max(Mathf.Abs(offset.x), Mathf.Abs(offset.y)) * Mathf.Max(width, depth);
				yieldEvery2 = Mathf.Max(yieldEvery2, approxNumNodesToUpdate / 10);
				int counter = 0;
				for (int z2 = 0; z2 < depth; z2++)
				{
					int xmin2;
					int xmax2;
					if (z2 >= recalculateRect.ymin && z2 < recalculateRect.ymax)
					{
						xmin2 = 0;
						xmax2 = width;
					}
					...
					{
						xmin2 = recalculateRect.xmin;
						xmax2 = recalculateRect.xmax;
					}
					for (int num7 = xmin2; num7 < xmax2; num7++)
					{
						graph.RecalculateCell(num7, z2, false, false);
					}
					counter += xmax2 - xmin2;
					if (counter > yieldEvery2)
					{
						counter = 0;
						yield return null;
					}
				...
				for (int z = 0; z < depth; z++)
				{
					int xmin;
					int xmax;
					if (z >= connectionRect2.ymin && z < connectionRect2.ymax)
					{
						xmin = 0;
						xmax = width;
					}
					...
					{
						xmin = connectionRect2.xmin;
						xmax = connectionRect2.xmax;
					}
					for (int num8 = xmin; num8 < xmax; num8++)
					{
						graph.CalculateConnections(num8, z);
					}
					counter += xmax - xmin;
					if (counter > yieldEvery2)
					{
			...
			int yieldEvery3 = Mathf.Max(depth * width / 20, 1000);
			int counter2 = 0;
			for (int z4 = 0; z4 < depth; z4++)
			{
				for (int num11 = 0; num11 < width; num11++)
				{
					graph.RecalculateCell(num11, z4);
				}
				counter2 += width;
			...
			for (int z3 = 0; z3 < depth; z3++)
			{
				...
				{
					graph.CalculateConnections(num12, z3);
				}
				counter2 += width;
				if (counter2 > yieldEvery3)
				{
