...
namespace Pathfinding
{
	...
	public abstract class AIBase : VersionedMonoBehaviour
	{
		...
		{
			// Guard: if no AstarPath exists in the scene (e.g. testing a non-school scene
			// or a scene where the graph isn't loaded yet), bail out cleanly instead of
			// throwing a NullReferenceException on every frame from Path.Reset().
