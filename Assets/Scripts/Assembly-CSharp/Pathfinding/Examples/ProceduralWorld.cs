	...
	public class ProceduralWorld : MonoBehaviour
	{
		...
		private class ProceduralTile
		{
			...
			private IEnumerator InternalGenerate()
			{
				...
				int counter = 0;
				float[,] ditherMap2 = new float[world.subTiles + 2, world.subTiles + 2];
				for (int i = 0; i < world.prefabs.Length; i++)
				{
					...
					for (int sx = 0; sx < world.subTiles; sx++)
					{
						for (int sz = 0; sz < world.subTiles; sz++)
						{
							...
							float fcount = subSize * subSize * density + ditherMap2[sx + 1, sz + 1];
							int count = Mathf.RoundToInt(fcount);
							ditherMap2[sx + 1 + 1, sz + 1] += 0.4375f * (fcount - (float)count);
							ditherMap2[sx + 1 - 1, sz + 1 + 1] += 0.1875f * (fcount - (float)count);
							ditherMap2[sx + 1, sz + 1 + 1] += 0.3125f * (fcount - (float)count);
							ditherMap2[sx + 1 + 1, sz + 1 + 1] += 0.0625f * (fcount - (float)count);
