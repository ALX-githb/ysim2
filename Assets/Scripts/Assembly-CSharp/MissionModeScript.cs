	...
	private void Update()
	{
		...
		else if (Phase == 2)
		{
			...
			if (NoCollateral)
			{
				if (Police.Corpses == 1)
				{
					if (StudentManager.Students[TargetID] != null && StudentManager.Students[TargetID].Alive)
					{
						Chastise = true;
						GameOverID = 5;
						GameOver();
			...
			if (TimeLimit)
			{
				...
				{
					TimeRemaining = Mathf.MoveTowards(TimeRemaining, 0f, 1f / 60f);
				}
