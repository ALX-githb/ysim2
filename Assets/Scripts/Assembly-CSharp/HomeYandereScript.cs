	...
	private void Update()
{
    if (!Disc.activeInHierarchy)
    {
        ...
        if (CanMove && MyController != null && MyController.enabled && MyController.gameObject.activeInHierarchy)
        {
            // Apply a gentle gravity effect if possible
            ...
            Vector3 moveDir = axis2 * right + axis * forward;
    ...
    else if (HomeDarkness != null && HomeDarkness.color.a == 0f)
    {
        ...
        else if (Timer > component2.clip.length + 1f)
        {
    ...
    // Reset Rigidbody velocity safely
    ...
    // Hair / debug shortcuts (unchanged)
    if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.H)) UpdateHair();
