...
public class BucketPourScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Yandere.Pouring)
		{
			if (PourHeight == "Low" && ControlFreak2.CF2Input.GetButtonDown("B"))
			{
				SplashCamera.Show = true;
				SplashCamera.MyCamera.enabled = true;
				SplashCamera.transform.position = new Vector3(2.875f, 0.8f, -35.625f);
				SplashCamera.transform.eulerAngles = new Vector3(0f, 45f, 0f);
			}
		}
		else if (Yandere.BucketDropping && ControlFreak2.CF2Input.GetButtonDown("B"))
		{
			SplashCamera.Show = true;
			SplashCamera.MyCamera.enabled = true;
			SplashCamera.transform.position = new Vector3(2.875f, 0.8f, -35.625f);
