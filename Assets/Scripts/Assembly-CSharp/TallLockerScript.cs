...
public class TallLockerScript : MonoBehaviour
{
		...
		if (Phase == 1)
		{
			...
			if (YandereLocker)
			{
				...
				if (Yandere.Bloodiness > 0f)
				{
					...
					{
						Object.Instantiate(BloodyClubUniform[(int)ClubGlobals.Club], Yandere.transform.position + Vector3.forward * 0.5f + Vector3.up, Quaternion.identity);
					...
					{
						Object.Instantiate(BloodyUniform[Yandere.PreviousSchoolwear], Yandere.transform.position + Vector3.forward * 0.5f + Vector3.up, Quaternion.identity);
	...
	public void SpawnSteam()
	{
		...
		else
		{
			...
			Student.Character.GetComponent<Animation>().CrossFade(Student.StripAnim);
