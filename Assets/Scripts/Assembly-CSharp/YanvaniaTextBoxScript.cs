			...
			if (BloodWipe.color.a <= 0f)
			{
				...
				else if (NewLabelScript != null)
				{
					...
					else
					{
						...
						if (NewLineTimer > component.clip.length + 0.5f || ControlFreak2.CF2Input.GetButtonDown("A") || ControlFreak2.CF2Input.GetKeyDown(KeyCode.Z) || ControlFreak2.CF2Input.GetKeyDown(KeyCode.X))
						{
							Display = false;
						}
					}
