...
public static class ConversationGlobals
{
	...
	{
		return GlobalsHelper.GetBool("TopicDiscovered_" + topicID);
	}
	...
	public static void SetTopicLearnedByStudent(int topicID, int studentID, bool value)
	{
		...
		GlobalsHelper.SetBool("TopicLearnedByStudent_" + text + '_' + text2, value);
	}
	...
	{
		Globals.DeleteCollection("TopicDiscovered_", KeysOfTopicDiscovered());
		IntAndIntPair[] array = KeysOfTopicLearnedByStudent();
		foreach (IntAndIntPair intAndIntPair in array)
		{
			Globals.Delete("TopicLearnedByStudent_" + intAndIntPair.first.ToString() + '_' + intAndIntPair.second.ToString());
		}
