...
public class BucketScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Yandere.PickUp != null)
		{
			...
			else if (Yandere.PickUp == PickUp && (Yandere.v != 0f || Yandere.h != 0f) && Full && ControlFreak2.CF2Input.GetButtonDown("RB"))
			{
				Yandere.EmptyHands();
				Yandere.Character.GetComponent<Animation>().CrossFade("f02_bucketTrip_00");
				Yandere.Tripping = true;
		...
		if (UpdateAppearance)
		{
			...
			Timer = Mathf.MoveTowards(Timer, 2f, Time.deltaTime);
			if (Timer == 2f)
			{
