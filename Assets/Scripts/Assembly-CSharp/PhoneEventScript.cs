...
public class PhoneEventScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (EventFriend.CurrentDestination != SpyLocation)
		{
			...
			{
				EventFriend.targetRotation = Quaternion.LookRotation(StudentManager.Students[33].transform.position - EventFriend.transform.position);
	...
	private void EndEvent()
	{
		...
		if (!EventOver)
		{
			...
			DumpPoint.enabled = false;
			DumpPoint.Prompt.Hide();
			DumpPoint.Prompt.enabled = false;
		}
		Yandere.Eavesdropping = false;
		EventActive = false;
		EventCheck = false;
